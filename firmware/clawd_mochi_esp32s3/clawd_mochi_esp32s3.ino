/*
 * ╔══════════════════════════════════════════════════════════════╗
 *   CLAWD MOCHI — ESP32-S3 Dev Board + ST7789 1.54" 240x240
 *
 *   Wiring:
 *     SDA / MOSI -> GPIO 10
 *     SCL / SCK  -> GPIO 9
...
int staStatusCode = WL_IDLE_STATUS;
...
// ── Colours ───────────────────────────────────────────────────
uint16_t C_ORANGE, C_DARKBG, C_MUTED, C_GREEN, C_PET;
...
void drawClawdFrame(const char* mood, int8_t eyeOffset, int8_t lid,
  ...
  tft.fillRoundRect(ox + 22, oy + 128, 76, 9, 4, C_DARKBG);
  tft.drawRoundRect(ox + 22, oy + 128, 76, 9, 4, C_MUTED);

  // Pixel-body Clawd: shadow, feet, torso, arms, eyes.
  tft.fillRect(bx + 3 * s,  by + 13 * s, s,     3 * s, C_PET);
  tft.fillRect(bx + 5 * s,  by + 13 * s, s,     3 * s, C_PET);
  tft.fillRect(bx + 9 * s,  by + 13 * s, s,     3 * s, C_PET);
  tft.fillRect(bx + 11 * s, by + 13 * s, s,     3 * s, C_PET);

  if (spark) {
    tft.fillRect(bx - s,       by + 8 * s, 2 * s, 2 * s, C_PET);
    tft.fillRect(bx + 14 * s,  by + 8 * s, 2 * s, 2 * s, C_PET);
  } else {
    tft.fillRect(bx,           by + 9 * s, 2 * s, 2 * s, C_PET);
    tft.fillRect(bx + 13 * s,  by + 9 * s, 2 * s, 2 * s, C_PET);
  }

  tft.fillRoundRect(bx + 2 * s, by + 6 * s, 11 * s, 7 * s, 3, C_PET);
  tft.fillRect(bx + 2 * s, by + 6 * s, 11 * s, s, C_DARKBG);

  if (lid > 8) {
    tft.fillRect(bx + 4 * s,  by + 9 * s, s, 2, C_BLACK);
    tft.fillRect(bx + 10 * s, by + 9 * s, s, 2, C_BLACK);
  } else if (mouth) {
    drawChevron(bx + 4 * s + 4,  by + 9 * s, 6, 6, 2, true,  C_BLACK);
    drawChevron(bx + 10 * s + 4, by + 9 * s, 6, 6, 2, false, C_BLACK);
  } else {
    tft.fillRect(bx + 4 * s,  by + 8 * s, s, 2 * s, C_BLACK);
    tft.fillRect(bx + 10 * s, by + 8 * s, s, 2 * s, C_BLACK);
  }
...
void drawClawdYawn(uint8_t frame) {
  ...
  tft.fillCircle(120, 124, 10 + frame % 2, C_BLACK);
...
void drawClawdError() {
  ...
  tft.drawLine(92, 98, 106, 112, C_BLACK);
  tft.drawLine(106, 98, 92, 112, C_BLACK);
  tft.drawLine(140, 98, 154, 112, C_BLACK);
  tft.drawLine(154, 98, 140, 112, C_BLACK);
  tft.drawCircle(120, 134, 9, C_BLACK);
  tft.drawCircle(120, 134, 10, C_BLACK);
}
...
void drawNetworkInfo() {
  ...
  tft.setTextSize(2);
  tft.setCursor(12, 16);  tft.print("WiFi setup");
  tft.setTextColor(C_MUTED);
  tft.setTextSize(1);
  tft.setCursor(12, 44);  tft.print("AP: ClaWD-Mochi");
  tft.setCursor(12, 58);  tft.print("pass: clawd1234");
  tft.setCursor(12, 76);  tft.print("setup: 192.168.4.1/wifi");
  tft.setTextColor(C_ORANGE);
  tft.setTextSize(1);
  if (staConnected) {
    tft.setCursor(12, 106); tft.print("LAN IP:");
    tft.setTextColor(C_WHITE);
    tft.setTextSize(2);
    tft.setCursor(12, 124); tft.print(WiFi.localIP());
    tft.setTextColor(C_MUTED);
    tft.setTextSize(1);
    tft.setCursor(12, 154); tft.print("http://clawd-mochi.local");
  } else {
    tft.setCursor(12, 100); tft.print("LAN: not connected");
    tft.setTextColor(C_WHITE);
    tft.setCursor(12, 118); tft.print("SSID: ");
    if (staSsid.length() > 0) {
      tft.print(staSsid.substring(0, 20));
    } else {
      tft.print("(not saved)");
    }
    tft.setCursor(12, 136); tft.print("status: ");
    tft.print(staStatusCode);
  }
}
...
void applyClaudeState(const String& state) {
  String s = state;
  s.trim();
  s.toLowerCase();
  ...
  if (s == "busy" || s == "working" || s == "thinking" || s == "typing" || s == "ultrathink") {
    ...
    drawIdleFace();
    Serial.println("CC:ok:done");
  } else if (s == "idle") {
    claudeVisualState = CC_IDLE;
    lastIdleBlinkMs = millis();
    drawIdleFace();
    Serial.println("CC:ok:idle");
  } else {
    Serial.print("CC:error:unknown:");
    Serial.println(s);
  }
}
...
void routeRedraw() {
  ...
  switch (currentView) {
    ...
    case VIEW_INFO:        drawNetworkInfo(); break;
  }
  server.send(200, "application/json", "{\"ok\":1}");
}
...
void routeClaudeState() {
  if (!server.hasArg("state")) {
    server.send(400, "application/json", "{\"ok\":0,\"error\":\"missing state\"}");
    return;
  }
  applyClaudeState(server.arg("state"));
  server.send(200, "application/json", "{\"ok\":1}");
}
...
void routeWifi() {
  if (server.hasArg("clear")) {
    ...
    server.send(200, "text/html",
      ...
      "<h2>已清除 WiFi</h2><p>设备正在重启。稍后连接热点 ClaWD-Mochi 重新配网。</p></body>");
    delay(300);
    ESP.restart();
    return;
  }
  ...
  if (server.hasArg("ssid")) {
    ...
    server.send(200, "text/html",
      "<!doctype html><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
      "<body style='font-family:sans-serif;background:#111;color:#fff;padding:24px'>"
      "<h2>已保存</h2><p>设备正在重启并连接 WiFi。稍后请打开 <b>http://clawd-mochi.local</b>。</p></body>");
    delay(300);
    ESP.restart();
    return;
  }

  String ip = staConnected ? WiFi.localIP().toString() : "not connected";
  int n = WiFi.scanNetworks();
  String options = "<option value=''>选择附近 WiFi</option>";
  for (int i = 0; i < n; i++) {
  ...
  String page =
    "<!doctype html><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'>"
    "<body style='font-family:sans-serif;background:#111;color:#fff;padding:24px'>"
    "<h2>Clawd Mochi 配网</h2>"
    "<p>当前局域网 IP：" + ip + "</p>"
    "<form method='get'>"
    "<p><select id='wifiList' style='font-size:18px;width:100%;padding:10px' onchange='document.getElementById(\"ssid\").value=this.value'>" + options + "</select></p>"
    "<p><input id='ssid' name='ssid' placeholder='WiFi 名称' style='font-size:18px;width:100%;padding:10px'></p>"
    "<p><input name='pass' placeholder='WiFi 密码' type='password' style='font-size:18px;width:100%;padding:10px'></p>"
    "<p><button style='font-size:18px;padding:10px 16px'>保存并重启</button></p>"
    "<p style='color:#aaa'>保存后设备会加入你的 WiFi。之后 Claude Code 可以通过局域网联动。</p>"
    "</form>"
    "<p><a href='/wifi?clear=1' style='color:#ff7a45'>忘记当前 WiFi / 重新配网</a></p>"
    "</body>";
...
void connectSavedWiFi() {
  ...
  WiFi.setHostname("clawd-mochi");
  WiFi.softAP(AP_SSID, AP_PASS);

  if (staSsid.length() == 0) {
    staStatusCode = WiFi.status();
    return;
  }
  ...
  staStatusCode = WiFi.status();
  staConnected = staStatusCode == WL_CONNECTED;
  if (staConnected) {
    MDNS.begin("clawd-mochi");
...
void routeState() {
  String j = "{\"device\":\"clawd-mochi\",\"view\":"; j += currentView;
...
void setup() {
  ...
  // ── Initial screen ─────────────────────────────────────────
  // Stay on the setup hint only when LAN is not ready yet.
  // Once WiFi succeeds, the device should feel alive immediately.
  if (staConnected) {
    drawIdleFace();
  } else {
    drawNetworkInfo();
  ...
  server.on("/cc",          HTTP_GET, routeClaudeState);
  server.on("/wifi",        HTTP_GET, routeWifi);
  server.on("/state",       HTTP_GET, routeState);
  server.onNotFound(routeNotFound);
  server.begin();

  // AP and /wifi stay available even after entering the idle face,
  // so the device can still be reconfigured flexibly.
}
  ...
  while (Serial.available()) {
    ...
    if (ch == '\n') {
      ...
      if (serialLine == "CC:ip") {
        drawNetworkInfo();
        Serial.print("CC:ip:");
        Serial.println(staConnected ? WiFi.localIP().toString() : "not-connected");
      } else if (serialLine.startsWith("CC:")) {
        applyClaudeState(serialLine.substring(3));
      } else if (serialLine.length() > 0) {
        Serial.print("CC:error:bad-command:");
      ...
      serialLine = "";
      Serial.println("CC:error:line-too-long");
    }
  }
}

void tickClaudeAnimations() {
